...
* `Numbered Bookmarks: List from All Files` List all bookmarks from all files
* `Numbered Bookmarks: Clear` remove all bookmarks from the current file
* `Numbered Bookmarks: Clear from All Files` remove all bookmarks from all files
...
List all bookmarks from the current file and easily navigate to anyone. It shows you the line contents and temporarily scrolls to that position.

### List from All Files

List all bookmarks from all files and easily navigate to anyone. It shows you the line contents and temporarily scrolls to that position.

![List](images/numbered-bookmarks-list-from-all-files.gif)

* Bookmarks from the active file show the line content and the position
* Bookmarks from other files also show the relative file path
...
Better yet, if you use the `numberedBookmarks.saveBookmarksInProject` setting defined as `true`, the bookmarks saved locally _will be available_ remotely, and you will be able to navigate and update the bookmarks. Just like it was a resource from a folder you opened remotely.

## Available Settings

* Bookmarks are always saved between sessions, and you can decide if it should be saved _in the Project_, so you can add it to your Git/SVN repo and have it in all your machines _(`false` by default)_. Set to `true` and it will save the bookmarks in the `.vscode\numbered-bookmarks.json` file.
```json
...
`allow duplicates` | You can have the same numbered bookmark in different files, and if you jump repeatedly to the same number, it will look at other files
...
* Choose the gutter icon to fill color
...
```json
    "workbench.colorCustomizations": {
      "numberedBookmarks.line background": "#157EFB22"  
    }
```
...
```json
    "workbench.colorCustomizations": {
      "numberedBookmarks.line border": "#FF0000"  
    }
```

* Choose a marker color to use in the overview ruler
```json
    "workbench.colorCustomizations": {
      "numberedBookmarks.overview ruler": "#157EFB88"  
    }
```
...
It also works even if you only _preview_ a file (simply click in TreeView). You can put bookmarks in any file and when you preview it again, the bookmarks will be there.
